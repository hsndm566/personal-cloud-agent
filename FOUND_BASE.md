...
Phase 0 selection is complete and the fork exists at https://github.com/hsndm566/personal-cloud-agent. The current fork main contains the read-only GitHub MCP hardening, Clerk JWT ownership boundary, and isolated LangGraph user-memory contract. Local verification on 2026-09-16: 219 tests passed and 4 skipped; Ruff passed and Pyrefly reported 0 errors. Supabase control-plane tables, pgmq dispatch, structured planning/verifier, version-controlled skill selection, a pinned Mem0 OSS adapter, an injectable LiteLLM alias gateway, a restart-safe queue worker boundary, and production Docker images for the API and standalone worker are implemented. Deep Agents execution is registered through the existing `deep-agent` boundary with read-only GitHub repository-inspection tools gated by the skill allowlist; `GITHUB_PAT` was not configured in the local test environment, so zero live GitHub tools loaded and no live GitHub MCP server was verified. Worker-to-agent wiring is implemented in `worker/agent_runner.py` and covered by restart/resume tests, but it has not been demonstrated against a live Supabase instance with a queued-to-running-to-completed transition. Northflank `personal-cloud-agent-api` is currently running with the fake model; Clerk environment variables and a real model-provider key have not been verified as configured there. Do not describe milestone 1 as complete until the supplied checklist is demonstrated.
